...
{
    private static function utility(): CatFactUtility
    {
        return new CatFactUtility();
    }
    ...
    private static function ctx(array $over = []): CatFactContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CatFactContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CatFactOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CatFactError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CatFactOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CatFactMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CatFactOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CatFactMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CatFactOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CatFactMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CatFactSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CatFactMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CatFactResponse([]),
            'result' => new CatFactResult([]),
        ]);
        [, $err] = CatFactMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CatFactSpec([]),
            'response' => null,
            'result' => new CatFactResult([]),
        ]);
        [, $err] = CatFactMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CatFactSpec(['step' => 's']),
            'response' => new CatFactResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CatFactResult([]),
        ]);
        [, $err] = CatFactMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CatFactSpec(['step' => 's']),
            'response' => new CatFactResponse(self::resp(200, ['v' => 1])),
            'result' => new CatFactResult([]),
        ]);
        [, $err] = CatFactMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CatFactSpec(['step' => 's']),
            'response' => new CatFactResponse(self::resp(200, ['v' => 2])),
            'result' => new CatFactResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CatFactMakeResponse::call($ctx);
    ...
    {
        $preset = new CatFactResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CatFactSpec([]),
            'response' => new CatFactResponse([]),
            'result' => new CatFactResult([]),
        ...
        [$got, $err] = CatFactMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CatFactResult([])]);
        [, $err] = CatFactMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CatFactSpec([]), 'result' => null]);
        [, $err] = CatFactMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CatFactSpec(['step' => 's']),
            'result' => new CatFactResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CatFactOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CatFactMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CatFactSpec(['step' => 's']),
            'result' => new CatFactResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CatFactOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CatFactMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CatFactResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CatFactSpec([]),
            'result' => new CatFactResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CatFactMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CatFactMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CatFactError('boom', 'boom');
        $utility->fetcher = function (CatFactContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CatFactSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CatFactMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CatFactContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CatFactMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CatFactResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CatFactContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CatFactSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CatFactMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CatFactResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CatFactSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CatFactMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CatFactMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CatFactMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CatFactResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CatFactDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CatFactResult(['ok' => false])]);
        $this->expectException(CatFactError::class);
        CatFactDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CatFactResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CatFactMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CatFactMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CatFactBaseFeature();
        $b = new CatFactBaseFeature();
        CatFactFeatureAdd::call($ctx, $a);
        CatFactFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CatFactBaseFeature
    {
        $f = new CatFactBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CatFactFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CatFactContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CatFactSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CatFactPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CatFactResponse(['headers' => null]),
            'result' => new CatFactResult([]),
        ]);
        CatFactResultHeaders::call($ctx);
