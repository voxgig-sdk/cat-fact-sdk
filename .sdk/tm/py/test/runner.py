...
class CatFactTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CatFactTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CatFactTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CatFactTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = CatFactTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CatFactTestRunner.getenv(key)
        ...
        explain = CatFactTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CatFactTestRunner._test_control is not None:
            return CatFactTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CatFactTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CatFactTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CatFactTestRunner.load_test_control()
...
def load_env_local():
    CatFactTestRunner.load_env_local()


def env_override(m):
    return CatFactTestRunner.env_override(m)


def entity_data(v):
    return CatFactTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CatFactTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CatFactTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CatFactTestRunner.load_test_control()


def live_client_options():
    return CatFactTestRunner.live_client_options()


def live_delay_ms():
    return CatFactTestRunner.live_delay_ms()
