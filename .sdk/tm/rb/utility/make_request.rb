...
module CatFactUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = CatFactResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CatFactResponse.new(fetched)
    else
