...
class FactEntity
  ...
  def data_set(args)
    if args
      @_data = CatFactHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CatFactHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Fact.
  #
  # @param reqmatch [FactLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Fact.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Fact, Hash] the loaded Fact; raises CatFactError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CatFactHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Fact>, Array] the matching Fact items; raises CatFactError on failure
