...
//     (\`new CatFactSDK(...)\` / \`CatFactSDK.test(...)\` become
//     \`CatFactSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"fact":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CatFactSDK(...)\` and any
// \`CatFactSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CatFactSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CatFactSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
