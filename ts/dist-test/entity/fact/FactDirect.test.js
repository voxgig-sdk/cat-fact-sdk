...
(0, node_test_1.describe)('FactDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-fact', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-fact', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.CAT_FACT_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CatFactSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.CAT_FACT_APIKEY,
        }));
        let idmap = env['CAT_FACT_TEST_FACT_ENTID'];
    ...
    const client = new __1.CatFactSDK({
        base: 'http://localhost:8080',
