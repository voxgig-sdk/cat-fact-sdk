# CatFact TypeScript SDK



The TypeScript SDK for the CatFact API — a type-safe, entity-oriented client with full async/await support.
...
```ts
const result = await client.fact.list()
...
```ts
const result = await client.fact.load({ id: 'example_id' })
...
| `tester(testopts?, sdkopts?)` | `CatFactSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CatFactSDK.test(testopts?, sdkopts?)` | `CatFactSDK` | Create a test-mode client. |
...
| `client` | `client(): CatFactSDK` | Return the parent SDK client. |
...
await fact.load({ id: "example_id" })

// fact.data() now returns the loaded fact data
// fact.match() returns { id: "example_id" }
```
