...
class FactEntity
{
    ...
    {
        if ($args) {
            $this->_data = CatFactHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = CatFactHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single Fact.
     *
     * @param FactLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FactLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Fact|array The loaded Fact as an assoc-array at the
     *   SDK boundary; throws CatFactError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CatFactHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws CatFactError on failure (item-5 convention).
     */
