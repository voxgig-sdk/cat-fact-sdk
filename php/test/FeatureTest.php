...
class FtCtrl extends CatFactControl
{
...
class FtClient
{
    ...
    private CatFactUtility $utility;

    public function __construct(CatFactUtility $utility, array $options)
    {
    ...
    public function get_utility(): CatFactUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CatFactContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CatFactContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CatFactConfig::make_feature($name);
    ...
    private static function buildUrl(CatFactSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CatFactError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CatFactResponse($fetched);
        }
    ...
    private function fail(CatFactContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CatFactContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
