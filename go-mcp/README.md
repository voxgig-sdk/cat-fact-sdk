...
[MCP](https://modelcontextprotocol.io) server exposing the CatFact SDK as
two agent tools — `cat-fact_list` and `cat-fact_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cat-fact-mcp)
make build

# 2. Provide credentials via the environment
export CAT_FACT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cat-fact \
  -- /absolute/path/to/cat-fact-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cat-fact-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cat-fact_list: first page of records
{ "entity": "fact" }
{ "entity": "fact", "query": { } }

// cat-fact_load: one record by id
{ "entity": "fact", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cat-fact-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cat-fact \
     -- "$PWD"/dist/*/cat-fact-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cat-fact_list` and `cat-fact_load` tools now appear
   in new sessions. Ask the agent to *"list fact using cat-fact"*
   and it calls `cat-fact_list` with `{"entity":"fact"}`.
...
```sh
export CAT_FACT_APIKEY=sk_live_xxx            # API key
export CAT_FACT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cat-fact_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cat-fact_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CAT_FACT_APIKEY` | API key sent with every request. |
| `CAT_FACT_BASE` | Optional override of the API base URL. |
...
```sh
./cat-fact-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cat-fact_load","arguments":{"entity":"fact","query":{"id":1}}}}'
```
