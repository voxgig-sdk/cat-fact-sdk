...
MCP server exposing the CatFact SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `cat-fact_list` | `entity`, optional `query` map | First page of records as JSON |
| `cat-fact_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `cat-fact_list` and
`cat-fact_load` tools become available in new sessions.
