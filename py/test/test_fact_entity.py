    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CATFACT_TEST_FACT_ENTID JSON to run live")
        client = setup["client"]
...
def _fact_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CATFACT_TEST_FACT_ENTID"))
    ...
    _live = env.get("CATFACT_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CATFACT_TEST_EXPLAIN") == "TRUE",
        "live": _live,
