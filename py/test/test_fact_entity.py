...
class TestFactEntity:
    ...
    def test_should_stream(self):
        ...
        base = CatFactSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CAT_FACT_TEST_FACT_ENTID JSON to run live")
        client = setup["client"]
        ...
        fact_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(fact_ref01_data_dt0_loaded))
...
def _fact_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CAT_FACT_TEST_FACT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("CAT_FACT_TEST_LIVE") == "TRUE":
        ...
        client = CatFactSDK(helpers.to_map(merged_opts))

    _live = env.get("CAT_FACT_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CAT_FACT_TEST_EXPLAIN") == "TRUE",
        "live": _live,
