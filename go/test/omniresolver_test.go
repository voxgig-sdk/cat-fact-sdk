...
import (
	...
	omni "github.com/voxgig-sdk/cat-fact-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CatFactError); is {
				out := map[string]any{
					"name":    "CatFactError",
...
func omniCtx(arg any, client *sdk.CatFactSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CatFactSDK); is {
			client = live
