...
func TestFactEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CAT_FACT_TEST_FACT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		// LOAD
		factRef01MatchDt0 := map[string]any{
			"id": factRef01Data["id"],
		}
		factRef01DataDt0Loaded, err := factRef01Ent.Load(factRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		factRef01DataDt0LoadResult := core.ToMapAny(entityData(factRef01DataDt0Loaded))
		if factRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if factRef01DataDt0LoadResult["id"] != factRef01Data["id"] {
...
func factBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CAT_FACT_TEST_FACT_ENTID")
