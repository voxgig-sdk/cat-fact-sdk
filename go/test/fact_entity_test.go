...
func TestFactEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATFACT_TEST_FACT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		factRef01DataDt0LoadResult := core.ToMapAny(factRef01DataDt0Loaded)
...
func factBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATFACT_TEST_FACT_ENTID")
