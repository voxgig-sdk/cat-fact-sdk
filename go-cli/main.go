// Command CatFact-cli is a CLI + REPL for the CatFact SDK,
...
const prompt = "cat-fact"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "fact user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CAT_FACT_APIKEY (api key), CAT_FACT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CAT_FACT_APIKEY carries the API key and
	// CAT_FACT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CAT_FACT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CAT_FACT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCatFactSDK(opts)
