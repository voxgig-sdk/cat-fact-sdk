...
boru-driven command-line client **and** interactive REPL for the CatFact
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cat-fact-cli)
make build
...
./cat-fact-cli load 1 fact            # {id:1} shorthand
./cat-fact-cli load '{id:1}' fact       # explicit match map
./cat-fact-cli list user

# 5. Override the API base URL for a single call
CAT_FACT_BASE=https://api.example.com ./cat-fact-cli list fact
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cat-fact-cli
   ```
...
```sh
./cat-fact-cli load 1 fact          # scalar shorthand for {id:1}
./cat-fact-cli load '{id:1}' fact     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `cat-fact>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `fact`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cat-fact-cli`. |
...
(`./cat-fact-cli <expr>`) and interactively (the REPL), and expressions compose the
